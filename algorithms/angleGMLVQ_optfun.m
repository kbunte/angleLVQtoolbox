function [f, G]  = angleGMLVQ_optfun(variables,trainX,LabelEqualsPrototype,beta,regularization,Lprototypes,Lrelevances,dim)
...
%     testx = [1,2,3,4];testw = [1,2,3,4];
%     testx = [NaN,NaN,3,NaN];
%     testd = testx(~isnan(testx))*testA(:,~isnan(testx))'*testA*testw'/(sqrt(testx(~isnan(testx))*testA(:,~isnan(testx))'*testA(:,~isnan(testx))*testx(~isnan(testx))')*sqrt(testw*testA'*testA*testw'));  
%     cell2mat(arrayfun(@(j) trainX(j,~isnan(trainX(j,:)))*A(:,~isnan(trainX(j,:)))'*A*w',1:3,'uni',0)')
%     arrayfun(@(j)sqrt(trainX(j,~isnan(trainX(j,:)))*A(:,~isnan(trainX(j,:)))'*A(:,~isnan(trainX(j,:)))*trainX(j,~isnan(trainX(j,:)))'),1:3)
...
%         else % long sample wise computation for testing
%             for acts=1:size(useX,1)
%                 dwJ = 2*distwrong(acts)/distcorrectpluswrong(acts)^2.*...
%                         (-beta/(exp(2*beta)-1)*exp(-beta*cosa(acts,pidxcorrect(acts))+beta) ) .* ...
%                         (useX(acts,:).*normw(pidxcorrect(acts))^2-w(pidxcorrect(acts),:).*xtw(acts,pidxcorrect(acts)))./(normx(acts)*normw(pidxcorrect(acts))^3);
%                 dwK =-2*distcorrect(acts)/distcorrectpluswrong(acts)^2.*...
%                         (-beta/(exp(2*beta)-1)*exp(-beta*cosa(acts,pidxwrong(acts))+beta) ) .* ...
%                         (useX(acts,:).*normw(pidxwrong(acts))^2-w(pidxwrong(acts),:).*xtw(acts,pidxwrong(acts)))./(normx(acts)*normw(pidxwrong(acts))^3);
%                 G(pidxcorrect(acts),2:end)= G(pidxcorrect(acts),2:end)+dwJ;
%                 G(pidxwrong(acts),2:end) =  G(pidxwrong(acts),2:end)+dwK;
%             end
%         end
...
%                 s=find(idxc==1,2);trainX(s(end),~isnan(trainX(s(end),:)))*A(:,~isnan(trainX(s(end),:)))'*A(:,~isnan(trainX(s(end),:)))
%                 t = nan(1,size(trainX,2));
%                 for i=1:size(trainX,2)
%                     for j=1:size(trainX,2)
%                         for d=1:2
%                             if ~isnan(trainX(s(end),i))
%                             t(j) = nansum([t(j),trainX(s(end),i)*A(d,i)*A(d,j)]);
%                             end
%                         end
%                     end
%                 end
%                 [useX(s(end),:)*A'*A;t]                
        ...
        if Lrelevances
            regTerm = zeros(length(relIdx),size(trainX,2));% normF3 = zeros(length(relIdx),size(trainX,2));
%             if length(relIdx)>1
%                 test = zeros(size(A));
%                 for s = 1:length(pidxcorrect)
%                     for d=1:size(A,1)
%                         for m=1:size(trainX,2)
%                             if ~isnan(trainX(s,m))
% test(d,m) = test(d,m) + mudJ(s)*dgwJ(s) * (...
%     (trainX(s,m)*(A(d,:)*w(pidxcorrect(s),:)') + w(pidxcorrect(s),m)*(A(d,:)*useX(s,:)'))/(normxA(s)*normwA(pidxcorrect(s))) - xAw(s,pidxcorrect(s)) * ...
%     (trainX(s,m)*(useX(s,:)*A(d,:)')/(normxA(s)^3*normwA(pidxcorrect(s))) + w(pidxcorrect(s),m)*(w(pidxcorrect(s),:)*A(d,:)')/(normxA(s)*normwA(pidxcorrect(s))^3))) ...
% 	 + mudK(s)*dgwK(s)*( ...
%     (trainX(s,m)*(A(d,:)*w(pidxwrong(s),:)') + w(pidxwrong(s),m)*(A(d,:)*useX(s,:)'))/(normxA(s)*normwA(pidxwrong(s))) - xAw(s,pidxwrong(s)) * ...
%     (trainX(s,m)*(useX(s,:)*A(d,:)')/(normxA(s)^3*normwA(pidxwrong(s))) + w(pidxwrong(s),m)*(w(pidxwrong(s),:)*A(d,:)')/(normxA(s)*normwA(pidxwrong(s))^3)));
% 
%                             end
%                         end
%                     end
...
for d=1:size(A,1)
    ...
    ga(d,:) = nansum( bsxfun(@times,(mudJ.*dgwJ)',dcosaJdA) + bsxfun(@times,(mudK.*dgwK)',dcosaKdA) );
                ...
                normF3 =  -4.*( 1-sum(arrayfun(@(d) sum(A(d,:).^2),1:dim)) ).* A; % -4*(1-sum(diag(A'*A))).*A;
...
%         %                 f3 =  regularization*(2./diag(A).*diag(A))';
%         %             normF3 = 2.*diag(A)';                    
%                 end
